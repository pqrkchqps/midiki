/**
 * Implements the NamedNodeMap interface for Midiki datatypes.
 */
/*
 ...
 */
public class MidikiNamedNodeMap implements org.w3c.dom.NamedNodeMap
{
    ...
    public Node item(int index) {
        if (_map==null) return null;
        if (index < 0 || index >= _map.entrySet().size()) return null;
        Node existing = (Node)((Map.Entry)_map.entrySet().toArray()[index]).getValue();
        return existing;
    }
    ...
    public Node removeNamedItem(String name) throws DOMException {
        if (_map==null) throw new DOMException(DOMException.NOT_FOUND_ERR, "Node not found");
        Node existing = (Node)_map.get(name);
        if (existing==null) throw new DOMException(DOMException.NOT_FOUND_ERR, "Node not found");
        return existing;
    }
    ...
    public Node setNamedItem(Node arg) throws DOMException {
        ...
        Node previous = (Node)_map.get(arg.getNodeName());
        _map.put(arg.getNodeName(), arg);
        return previous;
    }
