/**
 * Implements the Node interface for Midiki datatypes.
 *
 * The Node interface specification is copyrighted as follows:
...
import org.mitre.midiki.impl.mitre.*;  // desired methods not currently exported
import java.util.*;
 ...
 */
public class MidikiNode implements org.w3c.dom.Node {
    /**
     * An empty NamedNodeMap, as a singleton.
     */
    static protected MidikiNamedNodeMap emptyNNM = new MidikiNamedNodeMap();
    /**
     * Object whose structure is reflected by this DOM Node.
     */
    protected Object _shadow; // the underlying object
    /**
     * Parent DOM Node, if any.
     */
    protected MidikiNode _parent;
    /**
     * Children of this Node, if any.
     */
    protected MidikiNodeList _nodeList; // the children of this object, if needed
    /**
     *
     * @param shadow
     */
    public MidikiNode(Object shadow) {
        _shadow = shadow;
        // now see if we need to recursively create new nodes.
        // Following the logic in org.mitre.midiki.logic.Unifier,
        // we care if this object is a Predicate or a Collection,
        // in addition to caring about state-defining structures.
        if (shadow instanceof InfoState) {
            // I'd like to be able to query for which cells exist.
            // However, that isn't in the current interface spec.
            // We should be able to get a Contract for the InfoState;
            // that's reasonable and fits the model. But it means
            // making changes to the underlying interfaces and I'd
            // rather hold off on that for now. Instead, use the Impls.
            MidikiNodeList nodeList = (MidikiNodeList)getChildNodes();
            MidikiNode childNode = null;
            Set contractSet = ((InfoStateImpl)shadow).getContracts();
            Iterator cit = contractSet.iterator();
            while (cit.hasNext()) {
                Contract ctr = (Contract)cit.next();
                childNode = new MidikiCellNode(((InfoStateImpl)shadow).cell(ctr.name()));
                childNode._parent = this;
                nodeList.append(childNode);
            }
        } else if (shadow instanceof ImmutableInfoState) {
            //
            MidikiNodeList nodeList = (MidikiNodeList)getChildNodes();
            MidikiNode childNode = null;
            Set contractSet = ((ImmutableInfoStateImpl)shadow).getContracts();
            Iterator cit = contractSet.iterator();
            while (cit.hasNext()) {
                Contract ctr = (Contract)cit.next();
                childNode = new MidikiCellNode(((ImmutableInfoStateImpl)shadow).cell(ctr.name()));
                childNode._parent = this;
                nodeList.append(childNode);
            }
        } else if (shadow instanceof Cell) {
            MidikiNodeList nodeList = (MidikiNodeList)getChildNodes();
            MidikiNode childNode = null;
            Contract ctr = ((Cell)shadow).getContract();
            Iterator ait = ctr.attributes();
            while (ait.hasNext()) {
                Contract.Attribute attr = (Contract.Attribute)ait.next();
                if (((AttributeImpl)attr).type() instanceof Contract) {
                    childNode = new MidikiCellNode(((Cell)shadow).cell(attr.name()));
                } else {
                    Object arg = ((Cell)shadow).get(attr.name());
                    if (arg instanceof Predicate) {
                        childNode = new MidikiPredicateNode(arg);
                    } else if (arg instanceof Variable) {
                        childNode = new MidikiVariableNode(arg);
                    } else if (arg instanceof Collection) {
                        childNode = new MidikiCollectionNode(arg);
                    } else {
                        // if a String, make a #text node
                        //nodeList.append(new MidikiObjectNode(arg));
                        childNode = new MidikiTextNode(arg);
                    }
                }
                childNode._parent = this;
                nodeList.append(childNode);
            }
        } else if (shadow instanceof ImmutableCell) {
            MidikiNodeList nodeList = (MidikiNodeList)getChildNodes();
            MidikiNode childNode = null;
            Contract ctr = ((ImmutableCell)shadow).getContract();
            Iterator ait = ctr.attributes();
            while (ait.hasNext()) {
                Contract.Attribute attr = (Contract.Attribute)ait.next();
                if (((AttributeImpl)attr).type() instanceof Contract) {
                    childNode = new MidikiCellNode(((ImmutableCell)shadow).cell(attr.name()));
                } else {
                    Object arg = ((ImmutableCell)shadow).get(attr.name());
                    if (arg instanceof Predicate) {
                        childNode = new MidikiPredicateNode(arg);
                    } else if (arg instanceof Variable) {
                        childNode = new MidikiVariableNode(arg);
        ...
        } else if (shadow instanceof Predicate) {
            MidikiNodeList nodeList = (MidikiNodeList)getChildNodes();
            MidikiNode childNode = null;
            Iterator it = ((Predicate)shadow).arguments();
            while (it.hasNext()) {
                Object arg = it.next();
                if (arg instanceof Predicate) {
                    childNode = new MidikiPredicateNode(arg);
        ...
        } else if (shadow instanceof Collection) {
            MidikiNodeList nodeList = (MidikiNodeList)getChildNodes();
            MidikiNode childNode = null;
            Iterator it = ((Collection)shadow).iterator();
            while (it.hasNext()) {
                Object arg = it.next();
                if (arg instanceof Predicate) {
                    childNode = new MidikiPredicateNode(arg);
    ...
    /**
     * Return the object that this DOM Node reflects.
     */
    ...
    throws DOMException {
        // None of our node types has a value other than null, so NO-OP
    }
    
    /**
     * A code representing the type of the underlying object, as defined above.
     */
    protected short _nodeType;
    /**
     *
     * @return
     */
    ...
    public NodeList getChildNodes() {
        if (_nodeList == null) _nodeList = new MidikiNodeList();
        return _nodeList;
    }
    ...
    public Node getPreviousSibling() {
        ...
        return _parent._nodeList.getPrevious(this);
    }
    ...
    public Node getNextSibling() {
        if (_parent == null) return null;
        return _parent._nodeList.getNext(this);
    }
    ...
    public Node insertBefore(Node newChild,
            Node refChild)
            throws DOMException {
        throw new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR, "Cannot modify through this interface");
    }
    ...
    throws DOMException {
        ...
        MidikiNodeList nodeList = (MidikiNodeList)getChildNodes();
        //throw new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR, "Cannot modify through this interface");
        if (newChild.getNodeType() == org.w3c.dom.Node.DOCUMENT_FRAGMENT_NODE) {
            MidikiNodeList mnl = (MidikiNodeList)newChild.getChildNodes();
            MidikiNode mn = mnl.getFirst();
            while (mn != null) {
                nodeList.append(mn);
                mn = mnl.getNext(mn);
            }
        } else {
            nodeList.append((MidikiNode)newChild);
        }
        return newChild;
    }
    ...
    public boolean hasChildNodes() {
        return ((_nodeList != null) && (_nodeList.getLength() > 0));
    }
    ...
    public void normalize() {
        // MidikiDocuments have no Text nodes, so normalize() is a NO-OP.
    }
    ...
    public String getBaseURI() {
        return null; // not currently supporting DOM Level 3
    }
    ...
    throws DOMException {
        throw new DOMException(DOMException.NOT_SUPPORTED_ERR, "This operation is not yet implemented");
    }
    ...
    public boolean isSameNode(Node other) {
        // two MidikiNodes are considered the same if they are backed by the same object
        return (_shadow == ((MidikiNode)other)._shadow);
    }
    ...
    public boolean isDefaultNamespace(String namespaceURI) {
        return false; // not currently supporting DOM Level 3
    }
    ...
    public boolean isEqualNode(Node arg) {
        // two MidikiNodes are equal if their respective backing Objects are equal.
        return (_shadow.equals(((MidikiNode)arg)._shadow));
    }
    ...
    public Object setUserData(String key,
            Object data,
            Object /*UserDataHandler*/ handler) {  // UserDataHandler not in JDK 1.4.2; req 1.5 or higher
        return null; // not currently supporting DOM Level 3
    }
