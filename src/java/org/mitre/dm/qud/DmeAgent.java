...
public class DmeAgent implements Agent
{
    private static Logger logger =
        Logger.getLogger("org.mitre.dm.qud.DmeAgent");
    ...
    public void init(Object config)
    {
        ...
        try {
            fileHandler = new FileHandler("midiki_discourse.log");
            fileHandler.setLevel(Level.FINER);
            fileHandler.setFormatter(new XMLDiscourseFormatter());
            discourseLogger.addHandler(fileHandler);
        } catch (IOException ioe) {
            System.out.println("Can't initialize Midiki discourse log.");
        }
        */
        ...
        //queryDatabase = new QueryDatabase(discourseLogger);
    ...
    RemoveAccomplishedTask removeAccomplishedTask; // never used!
    Store store;
    StoreNIM storeNIM;
    ...
    static private long MAX_UPDATE_TIME = 5000L;
    ...
    private void update(InfoState infoState)
    {
        ...
        if (infoState.getUnifier().matchTerms(latest_speaker,"sys",bindings)) {
            ...
            while (canContinue() && refillAgenda.evaluate(infoState)); // not in original godis; trying to fix problems
            ...
            // remove irrelevant questions on agenda;
            // this is needed to handle responses to reqRep
            while (canContinue() && downdateAgenda.evaluate(infoState));
            // work on the plan
            if (agendaIsEmpty(infoState)) {
                while (canContinue() && managePlan.evaluate(infoState));
            }
            // refill agenda & clean up
            ...
            // terminate the user's turn in the log.
        ...
        //Object com = infoState.cell("is").cell("shared").get("com");
    ...
    public boolean connect(InfoState infoState)
    {
        ...
        /*
         * Set the rule to be fired when an agent makes a move.
         */
    ...
    public Set getRequiredContracts()
    {
        ...
        // need is and its components
        // need domain
        // need rec (make as a plain type?)
        // need database
        ...
        //contractSet.add(ContractDatabase.find("database"));
        return contractSet;
    }
    /**
