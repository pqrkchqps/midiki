...
public class Result
{
    ...
    public void shiftTopic(Object q, InfoState infoState, Bindings bindings)
    {
System.out.println("shift topic "+q);
        q = Unify.getInstance().deref(q, bindings);
System.out.println("shift topic 2 "+q);
        logger.logp(Level.FINER,className,"shiftTopic","shifting to topic",q);
        infoState.cell("is").cell("private").put("topic_shift", q);
    }
    ...
    public void removeCommonGround(Object p, InfoState infoState, Bindings bindings)
    {
        ...
        while (it.hasNext()) {
            Object item = it.next();
            if (infoState.getUnifier().matchTerms(oldcom,item,bindings)) {
                it.remove();
            }
        }
        infoState.cell("is").cell("shared").put("com", com);
    ...
    public void answerPlanQuestion(Object p, InfoState infoState, Bindings bindings)
    {
        ...
        if (planStack.isEmpty()) {
            logger.logp(Level.WARNING,className,"answerPlanQuestion","empty plan stack");
            return;
        }
    ...
    public void unanswerPlanQuestion(Object p, InfoState infoState, Bindings bindings)
    {
        Object planStackObj = infoState.cell("is").cell("private").get("plan");
        Stack planStack = (Stack)planStackObj;
        if (planStack.isEmpty()) {
        ...
        Object plan = planStack.peek();
        Object answer = infoState.getUnifier().deref(p,bindings);
        logger.logp(Level.FINER,className,"unanswerPlanQuestion","retracting answer",answer);
        //discourseLogger.logp(Level.FINER,className,"unanswerPlanQuestion","retracting answer",answer);
        LinkedList args = new LinkedList();
        args.add(plan);
        args.add(p);
        args.add(x);
        infoState.cell("domain").method("retract_answer").invoke(args,bindings);
    }

    public void unanswerPlanQuestion(Object p, Object q, InfoState infoState, Bindings bindings)
    {
        //Object answer = infoState.getUnifier().deref(p,bindings);
        Object answer = p;
        logger.logp(Level.FINER,className,"unanswerPlanQuestion","retracting answer",answer);
    ...
    public void popPlan(InfoState infoState, Bindings bindings)
    {
        logger.logp(Level.FINER,className,"popPlan","pop plan");
        Object planStack = infoState.cell("is").cell("private").get("plan");
        Object plan = ((Stack)planStack).pop();
        infoState.cell("is").cell("private").put("plan", planStack);
    }

    public void popToMatchingPlan(Object spec, InfoState infoState, Bindings bindings)
    {
        logger.logp(Level.FINER,className,"popPlan","pop plan");
        Object planStack = infoState.cell("is").cell("private").get("plan");
        Object plan = ((Stack)planStack).pop();
    ...
    public void instantiatePlan(Object plan, InfoState infoState, Bindings bindings)
    {
        ...
        plan = infoState.getUnifier().deref(plan,bindings);
        if (plan instanceof Variable) {
            throw new RuntimeException("Plan code is a variable!");
        }
        Object planStack = infoState.cell("is").cell("private").get("plan");
        if (planStack == null) planStack = new Stack();
        Variable planref = new Variable("PlanRef");
        LinkedList args = new LinkedList();
        args.add(plan);
        args.add(planref);
        infoState.cell("domain").method("instantiate_plan").invoke(args,bindings);
        ((Stack)planStack).push(infoState.getUnifier().deref(planref,bindings));
        infoState.cell("is").cell("private").put("plan", planStack);
    }
